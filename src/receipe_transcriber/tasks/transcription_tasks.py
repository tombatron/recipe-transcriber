...
# Flask app will be created lazily in the task to avoid circular imports
_app = None
...
def get_app():
    ...
    global _app
    if _app is None:
        from receipe_transcriber import create_app
        _app = create_app()
    return _app
...
def transcribe_recipe_task(self, job_id, image_path, urls, reprocess_recipe_id=None):
    """
    ...
    """
    logger.info(f"Job {job_id}: Starting transcription task (reprocess={reprocess_recipe_id is not None})")
    app = get_app()
    with app.app_context():
        try:
            ...
            publish_status(job, 'Starting transcription...', status='processing')

            if os.getenv('SKIP_OLLAMA') == '1':
                sleep(2)
                
            # Call Ollama service (the heavy lifting)
            publish_status(job, 'Sending image to model...')
            ...
            publish_status(job, 'Parsing response from model...')
